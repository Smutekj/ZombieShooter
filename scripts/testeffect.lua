...
Particles2 =
{
    ...
    spawner = function(spawn_pos, spawn_vel)
        ...
        p.pos = spawn_pos + dr;
        local sign = math.random(-1, 1)
        p.vel.x = 50. * (math.random() - 0.5);
        p.vel.y = 20.;

        p.life_time = 1.5;
        p.scale = Vec(5., 5.);

        p.color = Color(0, 1000, 0, 0);
...
local function drawShield(position, layers, radius, color, particles_color)
...
Effect.Drawer = function(effect, layers) 
    ...
    drawShield(effect.pos, layers, 10., c, eye_color);
end
